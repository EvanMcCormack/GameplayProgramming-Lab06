...
/// Didnt have time to add in the numbers at corners
/// </summary>
#include "MyVector3.h"
...
Vector3 Vector3::operator*(const double t_scalar) const
{
	return Vector3{ x * t_scalar, y * t_scalar, z * t_scalar };
}
/// <summary>
...
/// <returns>re-sized vector</returns>
Vector3 Vector3::operator/(const double t_divisor) const
{
	if (t_divisor != 0) // never divide by zero 
	{
		return Vector3{ x / t_divisor, y / t_divisor, z / t_divisor };
	}
	else
	{
		return Vector3{}; // dividing by zero has no effect according to Pete ??
	}
}
...
double Vector3::dot(const Vector3 t_other) const
{
...
double Vector3::angleBetween(const Vector3 t_other) const
{
	// formula
...
Vector3 Vector3::projection(const Vector3 t_other) const
{
...
Vector3 Vector3::rejection(const Vector3 t_other) const
{
